...
class myCallback(tf.keras.callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        if(logs.get('mae')< 0.06 and logs.get('val_mae')<0.06):
            print("\nMAE training and validation have been smaller than 0.6")
...
def windowed_dataset(series, batch_size, n_past=24, n_future=24, shift=1):
    ...
    ds = ds.window(n_future + n_past,shift=shift,drop_remainder=True)
...
def solution_C5():
    ...
    model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.Conv1D(filters=64, kernel_size=5, strides=1, padding='causal',
                               activation='relu', input_shape=[N_PAST, N_FEATURES]),
